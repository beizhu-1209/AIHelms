...
async function handleBatchAddMembers(): Promise<void> {
  if (!selectedDept.value || selectedUserIds.value.size === 0) return
  try {
...
const existingMemberIds = computed(() => new Set(members.value.map((m: DeptMember) => m.id)))
    ...
    <div class="flex-1 overflow-hidden rounded-2xl border border-slate-200/60 bg-white/70 shadow-sm backdrop-blur-xl">
      <template v-if="selectedDept">
        ...
        <div class="overflow-y-auto p-4" style="max-height: calc(100vh - 10rem)">
          <!-- 子部门列表 -->
          <div v-if="childDepts.length > 0" class="mb-4">
            <h4 class="mb-2 text-xs font-medium uppercase tracking-wide text-slate-500">下级部门</h4>
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
              <div
                v-for="child in childDepts"
                :key="child.id"
                class="flex cursor-pointer items-center gap-2 rounded-lg border border-slate-200/60 bg-slate-50/50 px-3 py-2 text-sm text-slate-700 transition-colors hover:border-purple-200 hover:bg-purple-50"
          ...
          <!-- 成员列表 -->
          <h4 v-if="childDepts.length > 0" class="mb-2 text-xs font-medium uppercase tracking-wide text-slate-500">部门成员</h4>
